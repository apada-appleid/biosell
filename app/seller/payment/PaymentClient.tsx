...
export default function PaymentClient() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) {
      alert('لطفاً یک تصویر با فرمت PNG یا JPG انتخاب کنید.');
      return;
    }
        ...
        <div className="bg-white shadow-lg rounded-lg overflow-hidden">
                    ...
                    <div className="mt-4">
                      ...
                      <div 
                        ...
                        className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition-colors ${
                          receiptImagePreview ? 'border-indigo-300 bg-indigo-50' : 'border-gray-300 hover:border-indigo-300'
                        }`}
                      >
                        {receiptImagePreview ? (
                      ...
                      {uploadStatus === 'error' && (
                        <p className="mt-2 text-xs text-red-600">خطا در بارگذاری تصویر. لطفاً دوباره تلاش کنید.</p>
                      )}
                    </div>
                  </div>
                )}
          ...
          <div className="p-6">
            ...
            <button
              ...
              disabled={isProcessing || (paymentMethod === 'transfer' && !receiptImage)}
            ...
            >
              {isProcessing ? (
                <span className="flex items-center justify-center">
                  ...
                  {uploadStatus === 'uploading' ? 'در حال بارگذاری فیش...' : 'در حال پردازش...'}
                </span>
              ) : (
                paymentMethod === 'online' ? 'پرداخت و تکمیل ثبت‌نام' : 'بارگذاری فیش و تکمیل ثبت‌نام'
              )}
            </button>
